...
/// Configuration for a tile layer's renderer which uses a canvas method to
/// paint tiles.
///
/// This is usually extended by a subclass more specific to a renderer.
@immutable
class CanvasRendererOptions {
  /// Create a configuration for a tile layer which uses a canvas method to
  /// paint tiles.
  ...
  /// ```
  ///
  /// `tileCoordinates` and `tileLoadStatus` are information about the tile
  ...
  /// by the renderer.
  ///
  /// `draw` should be called in the callback. It uses the `tilePaint` to draw
  ...
  ///
  /// ```dart
  /// paintTile: (canvas, size, tilePaint, tileCoordinates, tileLoadStatus,
  ///     draw) {
  ///   draw(
  ...
  /// ```
  ///
  /// Only calling `draw` without any arguments is equivalent to not setting
  /// this callback.
  final void Function(
    Canvas canvas,
    Size size,
    ...
    void Function({Rect? destRect}) draw,
  )? paintTile;
  ...
  bool operator ==(Object other) =>
      ...
      (other is CanvasRendererOptions &&
  ...
  @mustCallSuper
  @override
  int get hashCode => Object.hash(basePaint, paintTile);
}
