...
class ScaleLayerOption extends LayerOptions {
  TextStyle textStyle;
  Color lineColor;
  double lineWidth;
  final EdgeInsets padding;

  ScaleLayerOption({this.textStyle, this.lineColor = Colors.white, this.lineWidth = 2, this.padding});
}

class ScaleLayer extends StatelessWidget {
  final ScaleLayerOption scaleLayerOpts;
  final MapState map;
...
class ScalePainter extends CustomPainter {
  ...
  void paint(UI.Canvas canvas, UI.Size size) {
    ...
    double paddingRight = padding == null ? 0 : padding.right;
    double paddingTop = padding == null ? 0 : padding.top;
    double paddingBottom = padding == null ? 0 : padding.bottom;

    TextSpan textSpan = TextSpan(style: textStyle, text: text);
    TextPainter textPainter = TextPainter(text: textSpan, textDirection: TextDirection.ltr)..layout();
