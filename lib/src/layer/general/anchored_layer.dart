...
///
/// There are multiple ways to transform a widget into an anchored layer. In
/// order of preference:
///
...
///  3. Wrap the widget with an [AnchoredLayer] (which uses an extra widget that
///     mixes in [AnchoredLayerStatelessMixin])
///
...
///
///  - If you have control over the widget, do not use [AnchoredLayer]
/// in its `build` method. Prefer using the appropriate mixin, or wrap the
/// transformer around every instance of the widget.
///  - If the widget already uses a mixin, do not use [AnchoredLayer] in
/// addition. These widgets are designed to be used as an anchored layer only,
...
sealed class AnchoredLayer extends Widget {
  ...
  ///
  /// Uses an extra widget that mixes in [AnchoredLayerStatelessMixin]
  /// internally. Therefore, prefer another transformation method where possible:
  /// see documentation on [AnchoredLayer] for more information.
  ///
  /// ---
  ///
  /// {@macro anchored_layer_warning}
  const factory AnchoredLayer({Key? key, required Widget child}) =
...
///
///  1. A [Widget] is transformed into an [AnchoredLayer] by any means.
  ...
  static Widget _detectAncestor(BuildContext context) {
    context.visitAncestorElements((e) {
      ...
      throw FlutterError.fromParts([
        ErrorSummary(
          '`AnchoredLayer` (or one of the mixins) was used incorrectly.',
        ),
        ErrorDescription(
...
class _AnchoredLayerTransformer extends StatelessWidget
    with AnchoredLayerStatelessMixin
    implements AnchoredLayer {
  /// Reached externally by the constructor on [AnchoredLayer]
  const _AnchoredLayerTransformer({super.key, required this.child});
